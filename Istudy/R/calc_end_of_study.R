...
calc_end_of_study <- function(pheno_data,
                              study) {
    check_cols_exist(pheno_data, c("DATE_OF_BIRTH", "EXP_END"), "calc_end_of_study")  
    ...
    if(length(study@exp_len) == 1) {
        ...
        end_time <- exp_start_date + lubridate::dyears(pheno_data$EXP_END) + lubridate::years(study@wash_len + study@obs_len)
