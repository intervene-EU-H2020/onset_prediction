...
plt_d <- ggplot(cors_long, aes(x=abs(COR), y=reorder(Final_name, ENDPT_ORDER), color=Biobank)) +
...
plt_sup_c <- ggplot(cidx_wide, aes(x=C_IDX_base, y=C_IDX_PheRS, color=Endpoint)) +
...
```{r}
top_plt <- cowplot::plot_grid(NULL, plt_a, NULL, plt_sup_c, NULL, plt_d , nrow=1, align="h", rel_widths=c(0.025, 0.3, 0.025, 0.3, -0.025, 0.375))
top_plt
...
#ggsave(paste0(plt_res_dir, "fig5_v2.pdf"), full_plt, width=22, height=10, dpi=600, unit="in", bg="white")
```
