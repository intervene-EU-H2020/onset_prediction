...
#                              surv_ana) {
#     top_group <- dplyr::filter(coxph_hr_res, GROUP == get_comp_group(surv_ana@VAR_type, surv_ana@bin_cut))
...
plot_endpt_sd_hr <- function(coxph_hr_res,
    ...
    plt <- ggplot2::ggplot(top_group,
                            ...
                            labs(caption=paste0("Obs: ", surv_ana@study@obs_len, " Years until ", surv_ana@study@obs_end_date, " Wash: ", surv_ana@study@wash_len, " Years", get_surv_descr(surv_ana, surv_type="surv")),
                                 x="Hazard Ratio (95%)",
                                 y="") +
                            scale_color_manual(values=IUtils::custom_colors_brewer(length(unique(coxph_hr_res$VAR))), 
                                               name="Predictor") +
                            # Theme
                            IUtils::theme_custom(base_size=21) 
        if(length(unique(coxph_hr_res$VAR)) > 1) {
            ...
            plt <- plt + geom_point(show.legend = FALSE, size=3) + 
                    ...
                    labs(title=paste0(unique(coxph_hr_res$VAR), " 1-SD Increment"))
        }
...
plot_age_rg_hr <- function(surv_ana,
    ...
    comp_group <- get_comp_group(unique(coxph_hr_res$VAR))
    ...
    if(nrow(endpt_coxph_hr_res) > 0) {
        plt <- ggplot2::ggplot(endpt_coxph_hr_res, aes(x=as.character(EXP_AGE), y=HR)) +
                    ...
                    labs(title=endpt,
                         ...
                         caption=paste0("Exp: ", surv_ana@study@exp_len, 
                                        " Wash: ", surv_ana@study@wash_len, 
                                        " Obs: ",  surv_ana@study@obs_len, 
                                        " Years", get_surv_descr(surv_ana, surv_type="HR")),
                         x="Observation Age",
                         y="Hazard Ratio (95% CI)") +
                    # Theme
                    IUtils::theme_custom(base_size=21) +
